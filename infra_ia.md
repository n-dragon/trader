...
## 26. Conclusions générales (au terme de la v20, 9 juin 2026)

> Cette section répond directement à la demande : *« donne-moi tes conclusions »*, en synthétisant les vingt éditions et en intégrant la **concurrence sur le marché** et le **coût d'entrée** (la lentille douve B / concurrence C des v7→v20) ainsi que les **prix cibles** (v17→v20).

### 26.1 — Les sept enseignements structurants
...
7. **L'accès Euronext reste le facteur limitant pour un petit compte.** Sur ~140 noms cartographiés, l'essentiel des pépites est à Tokyo, en Corée, à Taïwan ou aux États-Unis. Les **vrais accès Euronext de cœur** sont peu nombreux : Nexans, Thales, Schneider, Air Liquide, STMicro, Legrand, Besi (Amsterdam), Prysmian (Milan), ASML/BESI, ArcelorMittal/Aperam (Amsterdam, GOES), 2CRSi (Paris, micro), et désormais **Heidelberg (Francfort)**.

### 26.2 — Le palmarès final par couple criticité × décote × douve (indice ≥ 6, tous tris confondus)
...
| 15-25 | **Tokyo Electron, Cleveland-Cliffs, EnerSys, AECOM, Thales, Ajinomoto, NRG, Dell, AT&S, TE Connectivity, Photronics** | — | **6** | WFE le moins cher / GOES US / batterie de salle / design DC / **HSM (Euronext)** / **monopole film ABF** / IPP gaz / pelle serveur / substrat EU / connecteur value / photomasques | Thales **oui (Paris)**, AT&S **oui (Vienne)** |

> **Note de lecture du palmarès.** Les **indices 8** restent le **noyau dur** : duopole EDA corrigé (Synopsys), mémoire mal-aimée (SK Hynix, Micron) et gaz-to-power sous-possédé (Energy Transfer). Les **indices 7** se sont **densifiés** au fil des éditions, ajoutant des **goulets de second rang à douve dure** (Onto sur l'inspection HBM4, Itron sur le réseau-software, Ibiden/Shin-Etsu sur les substrats/wafers). Le **seul pure-play Euronext de cœur dans le haut du tableau reste Nexans (7)** ; Thales et Ajinomoto (6) complètent côté accessible/monopole.

### 26.3 — Synthèse opérationnelle pour le compte (~1 068 €, eToro/Euronext)
- **Détenus à conserver/renforcer comme lignes infra-IA :** Air Liquide (6, gaz fabs + hélium), Schneider (5, power+cooling), STMicro (5, 800 VDC + SiPh). *Réévalués comme infra IA, pas comme défensives/cycliques.*
- **Top pick Euronext de rotation/ajout :** **Nexans (NEX.PA, 7)** — câble HV, *laggard* value, validé par le déficit cuivre + campus multi-GW + dorsale cuivre rack + délais réseau 4 ans.
- **Pépites Euronext de cœur (accessibles) :** **Thales (HO.PA, 6)** — HSM/sécurité, étiquette « défense » en retard ; **Heidelberg (HEI.DE, 6 effectif/5, nouveau v20)** — béton de l'IA, étiquette « ciment cyclique ». **Legrand (LR.PA)** — busways/PDU. **Besi (BESI.AS, 4)** & **Prysmian (PRY.MI, 5)** — sur repli (multiples pleins).
- **Watchlist US/Asie (eToro), par couple décote×douve :** Onto (7), Itron (7), Synopsys (8, sur maintien de la correction), Micron (8), NRG (6), Cisco (5), Fortinet (5), Cleveland-Cliffs (6), EnerSys (6), Tokyo Electron (6), Resonac (7), Renesas (5), Auras (5), Yageo (5), Neo (5), Vishay (5, cyclique).
- **Paris binaires/spéculatifs (taille minuscule) :** 2CRSi (5, two-phase), Silicom (4, DPU — louche), neoclouds (Nebius, ≤3).
- **À éviter / repli marqué seulement (rôle pricé / perfection) :** Arm, AMD, MP Materials, GE Vernova, Vertiv (multiple), Coherent, Lumentum, Camtek, Modine, Datadog, CrowdStrike, Tesla (VPP), Corning, Fabrinet, Quanta Services, Caterpillar.

> **Conclusion finale.** *En vingt éditions, la cartographie de l'infrastructure IA est devenue exhaustive — de l'atome (silice, GOES, NdFeB, film ABF) au modèle —, et le complexe s'est **largement revalorisé**. Le rendement ajusté du risque ne se trouve plus dans le « pas cher en absolu » (rare) mais dans **quatre poches récurrentes** : **(1)** les **dislocations de qualité** (mémoire HBM mal-aimée — SK Hynix, Micron) ; **(2)** les **dérivées énergie/réseau sous-possédées** (gaz-to-power ET/KMI/NRG, câble Nexans, transfo HD Hyundai, IPP Vistra, **réseau-software Itron**) ; **(3)** les **oligopoles/monopoles à étiquette en retard** (EDA Synopsys, HSM Thales, **film ABF Ajinomoto**, inspection Onto, fonderie TSMC, ciment Heidelberg) ; **(4)** les **consommables récurrents à douve de qualification** (Resonac, JX, Cohu, TOK). La discipline tient en cinq maximes cumulatives : **acheter la faiblesse** ; **offre contrainte ≠ surcapacité** ; **payer la douve, pas la perfection** ; **le rôle critique ne garantit pas la décote** ; et — ajout final — **plus la criticité d'un maillon est récente et médiatisée (terres rares, turbines, CPU-host agentique), plus son multiple est dangereux : la dernière décote structurelle est toujours le maillon dont personne n'a encore lu la bonne étiquette.** Tous les prix cibles et indices sont des **jugements d'analyse, sensibles au capex hyperscaler et aux multiples — non des garanties.***
